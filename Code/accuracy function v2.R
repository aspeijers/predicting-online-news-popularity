...
data1 <- read.csv("finalstretch_train.csv")
data2 <- read.csv("finalstretch_test.csv")
...
empty <- rep(0, nrow(train))
train <- cbind(train,empty)
empty2 <- rep(0, nrow(test))
test <- cbind(test, empty2)
colnames(train)[52] <- "transform"
colnames(test)[52] <- "transform"
...
accuracy <- function(X, Y, z = NULL){
	...
	actions <- c("scale", "log", "abs", "square", 
	...
	accuracy_measure[12] <- accuracy_base
	...
	test4[,z] <- abs(test4[,z])
	model_abs <- multinom(popularity ~ ., data = train4, maxit = 100, MaxNWts = 10000)
	...
	accuracy_measure[3] <- accuracy_abs
	accuracy_measure[4] <- accuracy_square
	accuracy_measure[5] <- accuracy_squaremean
	accuracy_measure[6] <- accuracy_squarex
	accuracy_measure[7] <- accuracy_cube
	accuracy_measure[8] <- accuracy_cubex
	accuracy_measure[9] <- accuracy_cubemean
	accuracy_measure[10] <- accuracy_cubesq
	accuracy_measure[11] <- accuracy_sqrtabs
...
train_round1 <- train[,-c(52)]
test_round1 <- test[,-c(52)]
# I hope it works
...
write.csv(train,"train_accuracyv2.csv")
write.csv(test,"test_accuracyv2.csv")

write.csv(train,"train_share.csv")
write.csv(test,"test_share.csv")