#Feature transform and select version 2 
...
empty <- rep(0, nrow(full_train))
full_train <- cbind(full_train,empty)
empty2 <- rep(0, nrow(full_test))
full_test <- cbind(full_test, empty2)
colnames(full_train)[61] <- "transform"
colnames(full_test)[61] <- "transform"
full_train <- as.data.frame(full_train)
full_test <- as.data.frame(full_test)
col <- colnames(full_train)

accuracy2 <- function(X, Y, z = NULL){
...
#The actual loop
for(i in 1:59){
	full_new <- accuracy2(full_train,full_test,i)
	full_train <- as.data.frame(full_new[1:nrow(full_train),])
	full_test <- as.data.frame(full_new[(nrow(full_train) +1):(nrow(full_train) + nrow(full_test)),])
	colnames(full_train) <- col
	colnames(full_test) <- col
}

#Checking the final accuracy
train_interimround <- full_train[,-c(61)]
test_interimround <- full_test[,-c(61)]
# I hope it works
model_interimround <- multinom(popularity ~ ., data = train_interimround)
results_interimround <- predict(model_interimround, test_interimround, "probs")
classes_interimround <- apply(results_interimround, 1, which.max)
accuracy_interimround <- (sum(classes_interimround == actualLabels))/9000

#Storing the new matrices
write.csv(train_interimround,"train_interimaccuracy.csv")
write.csv(test_interimround,"test_interimaccuracy.csv")
